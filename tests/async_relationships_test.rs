...
mod async_relationship_tests {
    use gouqi::relationships::{GraphOptions, IssueRelationships, RelationshipGraph};
    ...
    async fn debug_mock_response(jira: &Jira, issue_key: &str) {
        match jira.issues().get(issue_key).await {
            Ok(issue) => {
                println!(
                    "Successfully got issue: {} with {} fields",
                    issue.key,
                    issue.fields.len()
                );
    ...
    async fn test_async_get_relationship_graph_single_issue() {
        ...
        let result = jira
            .issues()
            .get_relationship_graph("PROJ-123", 0, None)
            .await;

        assert!(result.is_ok());
        let graph = result.unwrap();
        println!(
            "Graph metadata: source={}, issue_count={}, relationship_count={}",
    ...
    async fn test_async_get_relationship_graph_with_depth() {
        ...
        server
            .mock("GET", "/rest/api/2/issue/PROJ-124")
            .with_status(200)
            .with_header("content-type", "application/json")
        ...
        let result = jira
            .issues()
            .get_relationship_graph("PROJ-123", 1, None)
            .await;
    ...
    async fn test_async_relationship_graph_with_options() {
        ...
        let result = jira
            .issues()
            .get_relationship_graph("PROJ-123", 0, Some(options))
            .await;
    ...
    async fn test_async_relationship_graph_error_handling() {
        ...
        server
            .mock("GET", "/rest/api/2/issue/NONEXISTENT-123")
            .with_status(404)
            .with_header("content-type", "application/json")
        ...
        let result = jira
            .issues()
            .get_relationship_graph("NONEXISTENT-123", 1, None)
            .await;
    ...
    async fn test_async_relationship_graph_serialization() {
        ...
        let deserialized: RelationshipGraph =
