...
#[test]
fn test_count_with_auto_api_fallback() {
    let mut server = mockito::Server::new();

    // Mock v2 search endpoint for Auto fallback
    let mock_results = json!({
        "startAt": 0,
        "maxResults": 0,
        "total": 888,
    ...
    // Create client with Auto API version (will fallback to V2)
    let jira = Jira::with_search_api_version(
        server.url(),
        Credentials::Anonymous,
        gouqi::SearchApiVersion::Auto,
    )
    .unwrap();

    let count = jira.search().count("project = AUTO", false).unwrap();

    assert_eq!(count.count, 888);
    assert!(count.is_exact, "Auto fallback should use V2 exact count");
}

#[test]
fn test_count_with_auto_api_prefer_exact() {
    let mut server = mockito::Server::new();
    ...
    let count = jira.search().count("project = AUTO", true).unwrap();
...
mod async_count_tests {
    ...
    #[tokio::test]
    async fn test_async_count_auto_fallback() {
        let mut server = mockito::Server::new_async().await;
        ...
        let count = jira.search().count("project = AUTO", false).await.unwrap();

        assert_eq!(count.count, 555);
        assert!(
            count.is_exact,
            "Async Auto fallback should use V2 exact count"
        );
    }

    #[tokio::test]
    async fn test_async_count_auto_prefer_exact() {
        let mut server = mockito::Server::new_async().await;
        ...
        let count = jira.search().count("project = AUTO", true).await.unwrap();

        assert_eq!(count.count, 444);
        assert!(count.is_exact);
    }
}
