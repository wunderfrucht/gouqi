...
#[cfg(feature = "metrics")]
pub static METRICS: Lazy<GlobalMetrics> = Lazy::new(|| {
    GlobalMetrics::new()
});
...
impl GlobalMetrics {
    ...
    pub fn snapshot(&self) -> MetricsSnapshot {
        ...
        let endpoint_metrics = self.endpoint_metrics.read()
            .iter()
    ...
    fn record_request(&self, method: &str, endpoint: &str, duration: Duration, success: bool) {
        ...
        let endpoint_metrics = match metrics {
            ...
            None => {
                ...
                write_guard.entry(endpoint_key)
...
impl MetricsSnapshot {
    ...
    pub fn highest_error_endpoints(&self, min_requests: u64) -> Vec<(String, &EndpointMetricsSnapshot)> {
        let mut endpoints: Vec<_> = self.endpoints.iter()
...
impl MetricsCollector for GlobalMetrics {
    ...
    fn get_snapshot(&self) -> MetricsSnapshot { MetricsSnapshot::empty() }
    fn reset(&self) {}
}
