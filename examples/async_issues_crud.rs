//! Async Issues CRUD Operations Example
//!
//! This example demonstrates the comprehensive async CRUD operations available
...
#[cfg(feature = "async")]
use gouqi::{Credentials, Result, r#async::Jira};
#[cfg(feature = "async")]
...
async fn main() -> Result<()> {
    ...
    match issues.assign(issue_key, Some(username.to_string())).await {
    ...
    match issues.assign(issue_key, None).await {
    ...
    match issues.archive(issue_key).await {
    ...
    match issues.delete(issue_key).await {
    ...
    */
    ...
    match issues.get_watchers(issue_key).await {
        Ok(watchers) => {
            println!(
    ...
    match issues.add_watcher(issue_key, username.to_string()).await {
    ...
    match issues.remove_watcher(issue_key, username.to_string()).await {
    ...
    match issues.vote(issue_key).await {
    ...
    match issues.unvote(issue_key).await {
    ...
    fields1.insert(
        "summary".to_string(),
        serde_json::Value::String("Updated via async bulk operation".to_string()),
    );
    ...
    match issues.bulk_update(bulk_request).await {
        Ok(response) => {
    ...
    println!();
    println!("🎉 Async Issues CRUD operations demo completed!");
    ...
    println!("Run with: cargo run --example async_issues_crud --features async");
...
fn main() {
    ...
    eprintln!("Run with: cargo run --example async_issues_crud --features async");
}
